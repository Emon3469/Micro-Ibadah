...
export default function GroupCircle({ currentUser, onGroupJoined }) {
  ...
  return (
    <Card className="border-indigo-300 bg-linear-to-br from-indigo-50 via-white to-violet-50 shadow-md mt-6 mb-6 overflow-hidden">
      <CardHeader className="pb-3 border-b border-indigo-200/70">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-indigo-800/80 mt-1 flex items-center gap-1">
              Connect anonymously to boost Ibadah 
              <Badge variant="outline" className="bg-indigo-100 text-indigo-700 border-indigo-300 text-[10px] uppercase ml-2">Beta</Badge>
            </CardDescription>
          </div>
          
          <div className="flex gap-2">
            <Button size="sm" variant="outline" className="h-8 gap-1 bg-white text-indigo-800 hover:bg-indigo-100 border-indigo-300" onClick={() => { setIsJoining(true); setIsCreating(false); }}>
              <UserPlus className="h-3.5 w-3.5" /> Join
            </Button>
            <Button size="sm" className="h-8 gap-1 bg-indigo-700 hover:bg-indigo-800 text-white shadow-sm" onClick={() => { setIsCreating(true); setIsJoining(false); }}>
          ...
          <div className="flex gap-2 mt-4 overflow-x-auto pb-1 invisible-scrollbar">
            {groups.map(g => (
              <button 
                ...
                className={`px-3 py-1 text-xs font-semibold rounded-full whitespace-nowrap transition-colors ${activeGroup?._id === g._id ? 'bg-indigo-700 text-white shadow-sm' : 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200'}`}
              >
                {g.name}
              </button>
            ))}
      ...
      <CardContent className="pt-4 pb-0">
        ...
        {groups.length === 0 && !isCreating && !isJoining && (
          <div className="py-8 text-center px-4">
            <div className="mx-auto w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mb-3 border border-indigo-200">
              <Users className="h-6 w-6 text-indigo-600" />
            </div>
            <p className="text-sm text-indigo-900 font-semibold mb-1">No Friend Circles yet</p>
            <p className="text-xs text-indigo-700/80 max-w-62.5 mx-auto leading-relaxed">Create a circle and share the code with friends to track consistency together anonymously.</p>
          </div>
        )}
        ...
        {activeGroup && activeGroupDetails && !isCreating && !isJoining && (
          <div className="space-y-4 pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-white rounded-lg border border-indigo-200 shadow-sm">
              <div className="flex items-center gap-2 mb-2 sm:mb-0">
                <div className="w-8 h-8 rounded-full bg-indigo-200 flex items-center justify-center text-indigo-800 font-bold text-xs">{activeGroup.name.charAt(0)}</div>
                <div>
                  <h4 className="text-sm font-bold text-indigo-900">{activeGroupDetails.name}</h4>
                  <p className="text-[10px] text-indigo-700/80">{activeGroupDetails.members.length} member(s)</p>
                </div>
              </div>
              <div 
                onClick={copyInviteCode}
                className="flex items-center gap-2 px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 border border-indigo-200 cursor-pointer transition-colors rounded-md group"
              >
                <span className="text-xs font-semibold text-indigo-900 tracking-wider">CODE: <span className="font-mono text-indigo-700">{activeGroupDetails.inviteCode}</span></span>
                <Copy className="h-3 w-3 text-indigo-500 group-hover:text-indigo-800" />
              </div>
            </div>

            {/* Leaderboard */}
            <div className="bg-white rounded-xl border border-indigo-200 shadow-sm overflow-hidden">
              <div className="bg-linear-to-r from-indigo-100 to-violet-100 px-4 py-2 border-b border-indigo-200">
                <h5 className="text-xs font-semibold text-indigo-900 uppercase tracking-wider flex items-center gap-1.5"><Trophy className="h-3.5 w-3.5 text-amber-600" /> Hasanat Leaderboard</h5>
              </div>
              <div className="divide-y divide-indigo-100">
                {sortedMembers.map((member, index) => {
                  ...
                  return (
                    <div key={member._id} className={`flex items-center justify-between p-3 ${isMe ? 'bg-indigo-50/60' : ''}`}>
