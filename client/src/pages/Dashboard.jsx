...
export default function Dashboard() {
  ...
  useEffect(() => {
    const syncTheme = () => {
      setIsLast10Nights((localStorage.getItem("app_theme") || "ramadan") === "ramadan-night");
    };
    ...
    <div className={`${themeStyles} mx-auto w-full px-4 py-6 sm:px-6 lg:px-8`}>
      ...
      <Motion.header
        ...
        className={`mb-6 rounded-2xl border p-6 shadow-sm backdrop-blur-sm ${
          isNightTheme
            ...
            : "border-violet-300/80 bg-linear-to-br from-violet-100/80 via-[#f7f5f3] to-indigo-100/80 text-slate-800 shadow-[0_16px_32px_-20px_rgba(79,70,229,0.45)]"
        }`}
      >
        <div className="flex w-full items-center justify-between">
          <div className="flex items-center gap-4 sm:gap-5">
            <div
              className={`mt-1 flex h-20 w-20 shrink-0 items-center justify-center rounded-3xl ring-2 shadow-md overflow-hidden ${
                isNightTheme
                  ? "bg-linear-to-br from-primary/20 via-base-100 to-secondary/15 ring-primary/20"
                  : "bg-linear-to-br from-violet-100 via-base-100 to-indigo-100 ring-violet-300/70"
              }`}
            >
              <img src={projectLogo} alt="Micro-Ibadah logo" className="h-14 w-14 object-contain" />
            </div>
            <div>
          ...
          {currentUser && (
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 mr-4">
                <span className={`text-xs font-semibold uppercase tracking-wider ${isNightTheme ? 'text-indigo-300' : 'text-slate-600'}`}>Last 10 Nights</span>
                <button 
      ...
      {/* Laylatul Qadr Pinned Content */}
      {
        <Motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className={`mb-6 rounded-2xl border p-5 shadow-lg backdrop-blur ${
            isNightTheme
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <Sparkles className={`w-6 h-6 ${isNightTheme ? "text-yellow-400" : "text-amber-500"}`} />
            <h2 className="text-lg font-bold">✨ Laylatul Qadr Pinned Content</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className={`rounded-xl p-4 border ${isNightTheme ? "bg-white/10 border-white/10" : "bg-white/85 border-violet-200 shadow-sm"}`}>
              <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isNightTheme ? "text-indigo-300" : "text-indigo-700"}`}>Recommended Dua</p>
              <p className="text-xl font-arabic text-right mb-2" dir="rtl">اللَّهُمَّ إِنَّكَ عَفُوٌّ تُحِبُّ الْعَفْوَ فَاعْفُ عَنِّي</p>
              <p className={`text-xs italic ${isNightTheme ? "text-indigo-200" : "text-indigo-800/80"}`}>"O Allah, You are Pardoning and You love to pardon, so pardon me."</p>
            </div>
            <div className={`rounded-xl p-4 border space-y-2 ${isNightTheme ? "bg-white/10 border-white/10" : "bg-white/85 border-violet-200 shadow-sm"}`}>
              <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${isNightTheme ? "text-indigo-300" : "text-indigo-700"}`}>Checklist for Tonight</p>
              <div className="flex items-center gap-2 text-sm"><div className={`w-1.5 h-1.5 rounded-full ${isNightTheme ? "bg-yellow-400" : "bg-violet-500"}`} /> Pray Isha in congregation</div>
              <div className="flex items-center gap-2 text-sm"><div className={`w-1.5 h-1.5 rounded-full ${isNightTheme ? "bg-yellow-400" : "bg-violet-500"}`} /> Read at least 5 ayahs</div>
              <div className="flex items-center gap-2 text-sm"><div className={`w-1.5 h-1.5 rounded-full ${isNightTheme ? "bg-yellow-400" : "bg-violet-500"}`} /> Make dua for others</div>
            </div>
          </div>
        </Motion.div>
      }
      ...
      <main className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <section className="space-y-4 lg:col-span-2">
          ...
          {currentRamadanDay <= 30 ? (
            <Card className="border-indigo-300 bg-linear-to-br from-indigo-50 via-white to-violet-50 shadow-md mt-4 overflow-hidden">
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-indigo-900">🌙 Taraweeh Tracker</CardTitle>
                <CardDescription className="text-indigo-800/80">Did you pray Taraweeh tonight (Day {currentRamadanDay})?</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  <Button onClick={() => handleLogTaraweeh(8)} className="bg-indigo-700 hover:bg-indigo-800 text-white shadow-sm">Yes (8)</Button>
                  <Button onClick={() => handleLogTaraweeh(20)} className="bg-violet-700 hover:bg-violet-800 text-white shadow-sm">Yes (20)</Button>
                  <Button onClick={() => handleLogTaraweeh(2)} variant="outline" className="text-indigo-900 border-indigo-300 bg-indigo-100 hover:bg-indigo-200">Partially</Button>
                  <Button onClick={() => handleLogTaraweeh(0)} variant="outline" className="text-slate-700 border-slate-300 bg-white hover:bg-slate-100">No</Button>
                ...
                <div className="mt-4 pt-4 border-t border-indigo-300/60">
                  <p className="text-xs font-semibold text-indigo-900 mb-2 uppercase tracking-wide">30-Day Tracker</p>
                  <div className="grid grid-cols-10 gap-1 sm:flex sm:flex-wrap">
                    {Array.from({ length: 30 }).map((_, i) => {
                      ...
                      if (loggedDay?.logged) {
                        ...
                        if (loggedDay.count >= 8) color = "bg-indigo-700 border-indigo-700 shadow-sm";
                        else if (loggedDay.count > 0) color = "bg-violet-600 border-violet-600 shadow-sm";
                        else { color = "bg-slate-200 border-slate-300 opacity-70"; textColor = "text-slate-500"; }
                      }
                      if (day === currentRamadanDay && !loggedDay?.logged) {
                        color = "bg-amber-100 border-amber-400 animate-pulse";
