...
export default function Profile() {
  ...
  const [isNightTheme, setIsNightTheme] = useState(() => (localStorage.getItem("app_theme") || "ramadan") === "ramadan-night");
  ...
  useEffect(() => {
    const syncTheme = () => {
      setIsNightTheme((localStorage.getItem("app_theme") || "ramadan") === "ramadan-night");
    };
  ...
  if (loading) return (
    <div className={`min-h-screen flex items-center justify-center ${isNightTheme ? "bg-linear-to-br from-slate-950 via-slate-900 to-teal-950" : "bg-linear-to-br from-violet-50 to-purple-50"}`}>
  ...
  return (
    <div className={`min-h-screen py-6 px-4 ${isNightTheme ? "bg-linear-to-br from-slate-950 via-slate-900 to-teal-950" : "bg-linear-to-br from-[#fafafa] via-[#f5f5f5] to-[#f2f0ef]"}`}>
      <div className="max-w-2xl mx-auto space-y-6">
        {/* Profile Hero */}
        <Motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className={`rounded-2xl p-6 shadow-lg ${
            isNightTheme
              ? `bg-linear-to-r ${gradientClass} text-white`
              : "border border-violet-300/80 bg-linear-to-br from-violet-100/80 via-[#f7f5f3] to-indigo-100/80 text-slate-800 shadow-[0_16px_32px_-20px_rgba(79,70,229,0.45)]"
          }`}>
            <div className="flex items-center gap-4">
              <div className={`w-16 h-16 rounded-2xl backdrop-blur flex items-center justify-center text-3xl font-black shadow-inner ${
                isNightTheme ? "bg-white/20" : "bg-white/75 border border-violet-200"
              }`}>
                {user?.name?.charAt(0)?.toUpperCase() || "?"}
              </div>
              <div>
                <p className={`text-xs font-semibold uppercase tracking-wider ${isNightTheme ? "text-white/70" : "text-slate-600"}`}>Level — {currentLevel?.level}</p>
                <h1 className="text-xl font-black">{user?.nickname || user?.name}</h1>
                <p className={`text-sm ${isNightTheme ? "opacity-80" : "text-slate-600"}`}>{user?.department} · {user?.studentId}</p>
              </div>
              <div className="ml-auto text-right">
                <p className="text-3xl font-black">{rpgData?.user?.hasanatPoints || 0}</p>
                <p className={`text-xs ${isNightTheme ? "text-white/70" : "text-slate-600"}`}>Hasanat ⭐</p>
              </div>
            </div>
            ...
            {rpg && (
              <div className="mt-5">
                ...
                <div className={`w-full rounded-full h-2.5 overflow-hidden ${isNightTheme ? "bg-white/20" : "bg-slate-300/70"}`}>
                  <Motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${rpg.progressPct}%` }}
                    transition={{ duration: 1, ease: "easeOut" }}
                    className={`h-full rounded-full ${isNightTheme ? "bg-white" : "bg-violet-600"}`}
                  />
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={stat.label} className="rounded-xl bg-[#f7f5f3] border border-violet-200/70 p-3 text-center shadow-sm">
